...
class DriverShareTripRepo {
  final DioConsumer api;

  DriverShareTripRepo(this.api);
  ...
  Future<Either<Failure, CompleteTripResponseModel>>
      driverCompleteShareTripWithRiders({required int requestId}) async {
    try {
      final Response response = await api.post(
        "${EndPoints.driverCompleteShareTripWithRiders}/$requestId",
      );
      return Right(
        CompleteTripResponseModel.fromJson(
            response.data as Map<String, dynamic>),
      );
    } catch (e) {
      debugPrint(e.toString());
  ...
  Future<Either<String, String>> driverCancelScheduleTripForRider({
    ...
    try {
      final Response response = await api.post(
        "${EndPoints.driverCancelScheduleTripForRider}/$requestId",
