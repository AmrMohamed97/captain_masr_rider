...
import '../../data/repo/rider_share_trip_repo.dart';
import '../../../driver_trip/data/repo/driver_trip_repo.dart';

part 'rider_share_trip_state.dart';

class RiderShareTripCubit extends Cubit<RiderShareTripState> {
  RiderShareTripCubit({
    required int tripId,
    required int riderId,
  }) : super(RiderShareTripInitial()) {
  ...
  TripDetailsModel? currentRiderTrip;
    ...
    tripDetailsSubscription = dbTripDetails?.onValue.listen((event) {
      try {
        ...
        if (data != null) {
          ...
          emit(NewShareRequestsUpdateState());
          ...
          if (tripDetails?.status == "completed") {
            // final currentTripDetails = tripDetails;
            try {
              sl<Cache>().setData(
            ...
            emit(RideShareCompleteState(tripDetails: currentRiderTrip));
          }

          //! Cancel
          if (tripDetails?.status == "cancelled" ||
              tripDetails?.status == "canceled") {
            emit(RiderShareTripCancelledFromDriverState());
            return;
          }
  ...
  void bottomContainerExpandedToggle(bool value) {
    ...
    emit(RideShareBottomContainerExpandedToggleState());
  }
  ...
  Future<void> getRoute() async {
    emit(GetRouteShareLoadingState());
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        emit(GetRouteShareSuccessState());
  ...
  void _startStatusTimer() {
    ...
    if (isDriverWaiting || isTripStarted) {
      statusTimer = Timer.periodic(const Duration(seconds: 1), (timer) {
        ...
        emit(RideShareTimerState(
            counter: DateTime.now().millisecondsSinceEpoch));
        if (!kReleaseMode) {
          log(
            "State emitted - RideShareTimerState with counter: ${DateTime.now().millisecondsSinceEpoch}",
          );
        }
      });
      if (!kReleaseMode) {
    ...
    emit(RideShareTimerState(counter: DateTime.now().millisecondsSinceEpoch));
  }
  ...
  Future<void> cancelTrip({
    ...
    emit(RiderShareTripLoadingState());
    final result =
        // isShareRide ?
        await sl<RiderShareTripRepo>().cancelShareTrip(
      shareTripId: tripDetails?.id ?? 0,
      cancelReasons: cancelReasons,
      notes: notes,
    );
    // : await sl<RiderShareTripRepo>().cancelTrip(
    //     tripId: tripDetails?.rideId ?? 0,
    //     cancelReasons: cancelReasons,
    //     notes: notes,
    //   );
    result.fold(
      (error) => emit(RiderShareCancelTripErrorState(error: error)),
      (message) => emit(RiderShareCancelTripSuccessState(message: message)),
    );
  }
  ...
  Future<void> acceptScheduledTripRequest({required int requestId}) async {
    ...
    response.fold(
      (error) => emit(RiderShareCancelTripErrorState(error: error.toString())),
      (message) => emit(state),
    );
  }
